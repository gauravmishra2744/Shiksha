...
const puzzleTypes = {
  ...
  slidingPuzzle: {
    ...
    generator: (difficulty) => {
      ...
      const target = [[1, 2, 3], [4, 5, 6], [7, 8, 0]]; // 0 represents empty space
  ...
  pathfinding: {
    ...
    generator: (difficulty) => {
      ...
      // Add obstacles
      const obstacles = difficulty === "easy" ? 3 : difficulty === "medium" ? 5 : 8;
...
const LogicPuzzleGameContent = () => {
  ...
  const checkSolution = () => {
    ...
    if (currentPuzzle.type === "sudoku") {
      isCorrect = JSON.stringify(sudokuGrid) === JSON.stringify(currentPuzzle.solution);
    } else if (currentPuzzle.type === "sliding") {
      isCorrect = JSON.stringify(slidingGrid) === JSON.stringify(currentPuzzle.solution);
    ...
    if (isCorrect) {
      ...
      setCompletedPuzzles(prev => [...prev, { ...currentPuzzle, correct: true, attempts }]);
  ...
  const renderPuzzle = () => {
    ...
    switch (currentPuzzle.type) {
      ...
      case "pathfinding":
        return (
          <div className={`grid gap-1 max-w-sm mx-auto`} style={{gridTemplateColumns: `repeat(${currentPuzzle.size}, 1fr)`}}>
            {pathGrid.map((row, i) =>
              row.map((cell, j) => {
                const isInPath = playerPath.some(([pr, pc]) => pr === i && pc === j);
                const isStart = i === 0 && j === 0;
                const isEnd = i === currentPuzzle.size - 1 && j === currentPuzzle.size - 1;
                ...
                return (
                  <Button
                    ...
                    className={`aspect-square text-xs ${
                      isObstacle ? 'bg-gray-800 text-white' :
                      isStart ? 'bg-green-500 text-white' :
                      isEnd ? 'bg-red-500 text-white' :
                      isInPath ? 'bg-blue-500 text-white' : ''
                    }`}
                    onClick={() => handlePathMove(i, j)}
                    disabled={isObstacle}
                  >
                    {isStart ? 'S' : isEnd ? 'E' : isObstacle ? '■' : isInPath ? '●' : ''}
                  </Button>
                );
              })
            )}
              ...
              {currentPuzzle.pattern.map((item, index) => (
                <div
                  ...
                  className={`w-12 h-12 flex items-center justify-center rounded border-2 ${
                    item === "?" ? "border-blue-500 bg-blue-50 dark:bg-blue-950" : "border-gray-300"
                  }`}
                >
                  {item}
                </div>
          ...
          <Tabs defaultValue="sequence" className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <p className="text-center text-muted-foreground">
                    Complete patterns and find the next item in logical sequences
                  </p>
                  <div className="grid gap-4">
                    ...
                    >
                      <div className="text-center">
                        ...
                        <div className="text-sm opacity-90">Simple patterns • 15 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        ...
                        <div className="text-sm opacity-90">Complex sequences • 25 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        ...
                        <div className="text-sm opacity-90">Advanced logic • 40 points each</div>
                      </div>
                    </Button>
                  </div>
                </CardContent>
            ...
            <TabsContent value="sudoku" className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <p className="text-center text-muted-foreground">
                    Fill the 4x4 grid with numbers 1-4, no repeats in rows or columns
                  </p>
                  <div className="grid gap-4">
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Easy</div>
                        <div className="text-sm opacity-90">Few empty cells • 20 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Medium</div>
                        <div className="text-sm opacity-90">More challenges • 35 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Hard</div>
                        <div className="text-sm opacity-90">Minimal clues • 50 points each</div>
                      </div>
                    </Button>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid gap-4">
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Easy</div>
                        <div className="text-sm opacity-90">Few moves needed • 25 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Medium</div>
                        <div className="text-sm opacity-90">More scrambled • 40 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Hard</div>
                        <div className="text-sm opacity-90">Highly scrambled • 60 points each</div>
                      </div>
                    </Button>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid gap-4">
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Easy</div>
                        <div className="text-sm opacity-90">5x5 grid, few obstacles • 30 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Medium</div>
                        <div className="text-sm opacity-90">6x6 grid, more obstacles • 45 points each</div>
                      </div>
                    </Button>
                    <Button
                      size="lg"
                    ...
                    >
                      <div className="text-center">
                        <div className="font-bold">Hard</div>
                        <div className="text-sm opacity-90">7x7 grid, many obstacles • 65 points each</div>
                      </div>
                    </Button>
                  </div>
                </CardContent>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-sm font-medium capitalize">{puzzleTypes[puzzleType]?.name}</span>
                  </div>
          ...
          <Card className="border-2">
            <CardContent className="p-8 text-center space-y-6">
              {/* Instructions */}
              <div className="space-y-2">
                <h2 className="text-xl font-bold">{puzzleTypes[puzzleType]?.name}</h2>
                <p className="text-muted-foreground">{puzzleTypes[puzzleType]?.description}</p>
              </div>
              ...
              <div className="flex justify-center space-x-4">
                <Button onClick={checkSolution} className="px-8">
          ...
          <Card>
            <CardContent className="p-8 text-center space-y-6">
              <div className="space-y-2">
                ...
                <h2 className="text-2xl font-bold">Logic Challenge Complete!</h2>
